...
var n
return"string"==typeof i?"#"!=i[0]?(n=$("#"+i),n.length||(n=$(i))):n=$(i):n=i,n.length?(t.lastCtx=n,e instanceof WUX.WComponent?(e.mount(n),n.wuxComponent(e)):n.append(e),WUX.debug&&console.log("WuxDOM.mount "+WUX.str(e)+" on "+WUX.str(i)+" completed."),n):void console.error("WuxDOM.mount "+WUX.str(e)+" on "+WUX.str(i)+" -> context unavailable")},t.unmount=function(e){e||(e=t.lastCtx?t.lastCtx:$("#view-root")),WUX.debug&&console.log("WuxDOM.unmount "+WUX.str(e)+"...")
var i
if("string"==typeof e?"#"!=e[0]?(i=$("#"+e),i.length||(i=$(e))):i=$(e):i=e,!i.length)return void console.error("WuxDOM.unmount "+WUX.str(e)+" -> node unavailable")
...
e==this.state?t+='<li class="active"><a data-toggle="tab" class="active" href="#'+this.id+"-"+e+'" aria-expanded="true"> '+i.name+"</a></li>":t+='<li><a data-toggle="tab" href="#'+this.id+"-"+e+'"> '+i.name+"</a></li>"}t+="</ul>",t+='<div class="tab-content">'
...
this.root.find("tbody tr:eq("+o+")").addClass(this.selClass),this.state&&this.state.length>o&&e.push(this.state[o])}if(!this.handlers._selectionchanged)return this
...
i.addClass(this.selClass).siblings().removeClass(this.selClass),t.selectedRow=i.index()
...
break
case t.WInputType.CheckBox:if(!this.checkboxStyle){var O=Math.round(.8*parseInt(this.root.css("font-size")));(isNaN(O)||O<16)&&(O=16),this.checkboxStyle="height:"+O+"px;"}var k='<input type="checkbox" name="'+d.id+'" id="'+d.id+'" class="'+this.inputClass+'" '
this.checkboxStyle.length>2&&(k+='style="'+this.checkboxStyle+'" '),d.readonly&&(k+="readonly "),0==d.enabled&&(k+="disabled "),k+="/>",d.element=$(k)
break
case t.WInputType.Radio:d.component&&d.component.mount(f)
break
case t.WInputType.Select:var W=""
if(d.attributes?(W+='<select name="'+d.id+'" id="'+d.id+'" class="'+this.inputClass+'" '+d.attributes,(d.readonly||0==d.enabled)&&(W+=" disabled"),W+=">"):(W+='<select name="'+d.id+'" id="'+d.id+'" class="'+this.inputClass+'"',(d.readonly||0==d.enabled)&&(W+="disabled"),W+=">"),d.options&&d.options.length>0)for(var R=0;R<d.options.length;R++){var D=d.options[R]
W+="string"==typeof D?"<option>"+D+"</option>":'<option value="'+D.id+'">'+D.text+"</option>"}W+="</select>",d.element=$(W)
break
...
this.root.append(F)
var N=$('<div class="'+this.footerClass+'"'+M+"></div>")
F.append(N)
for(var P=0,H=this.footer;P<H.length;P++){var V=H[P]
"string"==typeof V&&V.length>0?N.append($(V)):V instanceof t.WComponent?V.mount(N):N.append(V)}}for(var B=this,G=0,j=this.dpids;G<j.length;G++){var X=j[G]
