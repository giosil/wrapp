...
$.when($.getJSON(n+"ca-gregorian.json"),$.getJSON(n+"numbers.json"),$.getJSON(n+"currencies.json"),$.getJSON(n+"timeZoneNames.json"),$.getJSON(n+"supplemental/likelySubtags.json"),$.getJSON(n+"supplemental/timeData.json"),$.getJSON(n+"supplemental/weekData.json"),$.getJSON(n+"supplemental/currencyData.json"),$.getJSON(n+"supplemental/numberingSystems.json"),$.getJSON(n+"supplemental/plurals.json"),$.getJSON(n+"supplemental/ordinals.json")).then(function(){return[].slice.apply(arguments,[0]).map(function(t){return t[0]})}).then(Globalize.load).then(function(){Globalize.locale(t.global.locale),DevExpress.localization.locale(t.global.locale),DevExpress.config({defaultCurrency:"EUR"}),t.debug&&console.log("[WUX] global.init completed"),e&&e()}).fail(function(){t.debug&&console.log("[WUX] global.init failed")})},setData:function(t,e,i){if(void 0===i&&(i=!1),t||(t="global"),A[t]=e,!i&&L[t])for(var n=0,o=L[t];n<o.length;n++){var r=o[n]
