...
break
case t.WInputType.CheckBox:if(!this.checkboxStyle){var O=Math.round(.8*parseInt(this.root.css("font-size")))
O<16&&(O=16),this.checkboxStyle="height:"+O+"px;"}this.checkboxStyle.length>2?d.element=$('<input type="checkbox" name="'+d.id+'" id="'+d.id+'" class="'+this.inputClass+'" style="'+this.checkboxStyle+'"/>'):d.element=$('<input type="checkbox" name="'+d.id+'" id="'+d.id+'" class="'+this.inputClass+'"/>')
break
case t.WInputType.Radio:d.component&&d.component.mount(f)
break
case t.WInputType.Select:var k=""
if(d.attributes?(k+='<select name="'+d.id+'" id="'+d.id+'" class="'+this.inputClass+'" '+d.attributes,(d.readonly||0==d.enabled)&&(k+=" disabled"),k+=">"):(k+='<select name="'+d.id+'" id="'+d.id+'" class="'+this.inputClass+'"',(d.readonly||0==d.enabled)&&(k+="disabled"),k+=">"),d.options&&d.options.length>0)for(var W=0;W<d.options.length;W++){var D=d.options[W]
k+="string"==typeof D?"<option>"+D+"</option>":'<option value="'+D.id+'">'+D.text+"</option>"}k+="</select>",d.element=$(k)
break
case t.WInputType.Email:var R='<input type="text" name="'+d.id+'" id="'+d.id+'" class="'+this.inputClass+'" '
d.readonly&&(R+="readonly "),0==d.enabled&&(R+="disabled "),d.style&&(R+=' style="'+t.style(d.style)+'"'),R+="/>",d.element=$(R)
break
case t.WInputType.Ftp:var I='<input type="text" name="'+d.id+'" id="'+d.id+'" class="'+this.inputClass+'" '
d.readonly&&(I+="readonly "),0==d.enabled&&(I+="disabled "),d.style&&(I+=' style="'+t.style(d.style)+'"'),I+="/>",d.element=$(I)
break
case t.WInputType.Password:var E='<input type="password" name="'+d.id+'" id="'+d.id+'" class="'+this.inputClass+'" '
d.readonly&&(E+="readonly "),0==d.enabled&&(E+="disabled "),d.style&&(E+=' style="'+t.style(d.style)+'"'),E+="/>",d.element=$(E)
break
case t.WInputType.Cron:var A=""
null!=d.value&&""!=d.value&&(A='initial: "'+d.value+'" ,')
var L='<div id="cron_'+d.id+'" class="'+this.inputClass+'" '
d.readonly&&(L+="readonly "),0==d.enabled&&(L+="disabled "),L+=" ></div>",L+='<script type="text/javascript">$(document).ready(function() {',L+='$("#cron_'+d.id+'").cron({'+A+'onChange: function() {$("#'+d.id+'").val($(this).cron("value"));},useGentleSelect: true});',L+='});<\/script><input type="hidden" name="'+d.id+'" id="'+d.id+'" />',d.element=$(L)
break
...
i.indexOf(",00")>=0&&i.indexOf(",00")==i.length-3&&(i=i.substring(0,i.length-3)),i.indexOf(",0")>=0&&i.indexOf(",0")==i.length-2&&(i=i.substring(0,i.length-3)),this.value=i,$(this).select()}}),"c"==d.valueType?d.element.blur(function(e){this.value&&(this.value=t.formatCurr(t.WUtil.toNumber(this.value)))}):d.element.blur(function(e){this.value&&(this.value=t.formatCurr5(t.WUtil.toNumber(this.value)))})):"n"==d.valueType?(d.element.focus(function(e){this.value&&(this.value=t.formatNum(this.value),$(this).select())}),d.element.blur(function(e){this.value&&(this.value=t.formatNum(this.value))})):"i"==d.valueType?(d.element.focus(function(e){this.value&&(this.value=t.WUtil.toInt(this.value),$(this).select())}),d.element.blur(function(e){this.value&&(this.value=t.WUtil.toInt(this.value))})):d.element.focus(function(t){$(this).select()})),null==d.visible||d.visible||(d.element.hide(),d.label&&$('label[for="'+d.id+'"]').hide()),d.onmount&&d.onmount(d),d.onfocus&&d.element.focus(d.onfocus),d.onblur&&d.element.focus(d.onblur))}}}for(var U=0,M=this.hiddens;U<M.length;U++){var d=M[U]
null==d.value&&(d.value=""),this.root.append('<input type="hidden" name="'+d.id+'" id="'+d.id+'" value="'+d.value+'">')}if(this.footer&&this.footer.length>0){var F=t.style(this.footerStyle)
if(this.footerClass||(this.footerClass="col-md-12"),F=F?' style="'+F+'"':' style="text-align:right;"',this.footerSep)if("string"==typeof this.footerSep){var N=this.footerSep.charAt(0)
"<"==N?this.root.append(this.footerSep):t.WUtil.isNumeric(N)&&this.root.append('<div style="height:'+this.footerSep+'px;"></div>')}else this.root.append('<div style="height:'+this.footerSep+'px;"></div>')
var P=$('<div class="row"></div>')
this.root.append(P)
var H=$('<div class="'+this.footerClass+'"'+F+"></div>")
P.append(H)
for(var V=0,B=this.footer;V<B.length;V++){var G=B[V]
"string"==typeof G&&G.length>0?H.append($(G)):G instanceof t.WComponent?G.mount(H):H.append(G)}}for(var j=this,z=0,X=this.dpids;z<X.length;z++){var K=X[z]
!function(i){var n=t.WUtil.getDate(j.minValues,i)
...
p>=19000101&&u>p&&(t.showWarning(t.RES.ERR_DATE),$(n.target).datepicker("setDate",l))}}}})}(K)}"object"==typeof this.state&&this.updateView(),this.root.find("input").keypress(function(t){if(13==t.which){for(var i=$(t.target).attr("id"),n=e.next(i);n&&!e.isFocusable(n);)n=e.next(n.id)
