...
},'Insertar una validación de datos');
jQuery.validator.addMethod("pwd",function(val,ele){
	...
	return true;
},'Ingresar contraseña valida (lung=8, al menos 1 let. min, 1 let. may., 1 num.)');
jQuery.validator.addMethod("notEqualTo",function(val,ele,par){
	if(this.optional(ele))return true;
	if(par && par.indexOf('#')==0)return val!=$(par).val();
	return val!=par;
},'Valor introducido no permitido');
...
function _confirm(msg,f){
	if(typeof f==='function'){
		swal({title:"Confirmación",type:"warning",text:msg,confirmButtonText:"Si",cancelButtonText:"No",confirmButtonColor:"#dd6b55",showCancelButton:true,closeOnConfirm:true,closeOnCancel:true},f);
	}
	else{
...
function _getInput(msg,f,d) {
	if(typeof f==='function'){
		swal({title:"Insertar",type:"input",inputValue:d,text:msg,showCancelButton:true},f);
	}
	else {
...
function updatePassword(){
	...
	$.ajax({
		...
		success: function(res) {
			$("#dlg-cp").modal('hide');
			_showSuccess("Contraseña actualiza");
			$('#cpop').val('');
			$('#cpnp').val('');
			$('#cpcp').val('');
		},
		error: function(xhr) {
			_showWarning("Contraseña invalida");
			$('#cpop').focus();
		}
	});
};
