namespace GUI {
	...
	export class GUIStructures extends WUX.WComponent {
		...
		protected render() {
			...
			this.btnFind.on('click', (e: JQueryEventObject) => {
				...
				jrpc.execute('STRUCTURES.find', [this.fpFilter.getState()], (result: any[]) => {
			...
			this.fpFilter.addBooleanField(SYM_E.AGRE, I18.L_AGRE);
			...
			this.btnSave.on('click', (e: JQueryEventObject) => {
				...
				jrpc.execute('STRUCTURES.exists', [cf, id], (result) => {
					if (result) {
						this.btnSave.blur();
					...
					if (this.isNew) {
						jrpc.execute('STRUCTURES.insert', [values], (result) => {
							this.status = this.iSTATUS_VIEW;
						...
						jrpc.execute('STRUCTURES.update', [values], (result) => {
							this.status = this.iSTATUS_VIEW;
							this.fpDetail.enabled = false;
							WUX.showSuccess(I18.M_OK);
			...
			this.fpDetail.addBooleanField(SYM_E.AGRE, I18.L_AGRE);
			...
			this.tabResult.onSelectionChanged((e: { element?: JQuery, selectedRowsData?: Array<any> }) => {
				...
				jrpc.execute('STRUCTURES.read', [id], (result) => {
					this.fpDetail.clear();
