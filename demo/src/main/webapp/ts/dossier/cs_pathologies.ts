namespace GUI {
	...
	export class GUIPathologies extends WUX.WComponent {
		...
		protected render() {
			...
			this.btnFind.on('click', (e: JQueryEventObject) => {
				...
				jrpc.execute('PATHOLOGIES.find', [this.fpFilter.getState()], (result: any[]) => {
					this.status = this.iSTATUS_VIEW;
					this.tabResult.setState(result);
					if (this.selId) {
						let idx = WUtil.indexOf(result, SYM_E.CODE, this.selId);
						if (idx >= 0) {
							setTimeout(() => {
			...
			this.btnSave.on('click', (e: JQueryEventObject) => {
				...
				let id = WUtil.getString(values, SYM_E.ID);
				let cf = WUtil.getString(values, SYM_E.CODE);
				jrpc.execute('PATHOLOGIES.exists', [cf, id], (result) => {
					if (result) {
						this.btnSave.blur();
					...
					if (this.isNew) {
						jrpc.execute('PATHOLOGIES.insert', [values], (result) => {
							this.status = this.iSTATUS_VIEW;
							this.fpDetail.enabled = false;
							WUX.showSuccess(I18.M_OK);
							
							this.selId = result[SYM_E.ID];
							this.fpFilter.clear();
							this.fpFilter.setValue(SYM_E.CODE, result[SYM_E.CODE]);
							this.btnFind.trigger('click');
						});
					}
					else {
						jrpc.execute('PATHOLOGIES.update', [values], (result) => {
							this.status = this.iSTATUS_VIEW;
							this.fpDetail.enabled = false;
							WUX.showSuccess(I18.M_OK);
			...
			this.tabResult.onSelectionChanged((e: { element?: JQuery, selectedRowsData?: Array<any> }) => {
				...
				let id = WUtil.getString(item, SYM_E.ID);
				if (!id) return;

				jrpc.execute('PATHOLOGIES.read', [id], (result) => {
					this.fpDetail.clear();
